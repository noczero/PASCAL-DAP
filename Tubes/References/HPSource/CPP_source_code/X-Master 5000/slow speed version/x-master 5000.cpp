...
void cGame::m_Jump()
{
	...
	if(m_nJump==1)
	{
		m_nJumpS+=1;
		m_nGravOff=1;
		if(m_nJumpS==15)//controls up-time of jump--- (ascend)
		{//											|
			if(y>m_nJlimit){//						|
		...
		//							                |
		if(m_nJumpS==45)//controls down-time of jump| (descend)
		{
	...
	//----------------------------TRAP------------>
			if(y==17 && x>34 && x<39)
			{
				if(m_nTrap1!=-1 && m_nTrap2!=-1 && m_nTrap3!=-1 && m_nTrap4!=-1){ 
					if(m_nTrap1!=400 || m_nTrap2!=400 || m_nTrap3!=400 || m_nTrap4!=400)
						m_nDown=1;
					if(x==35)
						m_nTrap1+=1;
					if(x==36)
